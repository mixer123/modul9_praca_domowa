...
def load_all_csv_from_do(bucket_name: str, prefix: str = "") -> pd.DataFrame:
    try:
        response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=prefix)
        if "Contents" not in response:
            st.warning("⚠️ Brak plików w przestrzeni DO.")
            return pd.DataFrame()

        all_dfs = []
        for obj in response["Contents"]:
            file_name = obj["Key"]
            if file_name.endswith(".csv"):  # tylko CSV
        ...
        st.error(f"❌ Błąd podczas pobierania plików: {e}")
        return pd.DataFrame()

print(load_all_csv_from_do(DO_SPACE_NAME))
